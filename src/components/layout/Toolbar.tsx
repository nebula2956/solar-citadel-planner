...
export function Toolbar({ onExportPng, onShare, onSave, sidebarOpen, onToggleSidebar, membersOpen, onToggleMembers, isMobile }: Props) {
  ...
  const LogoSvg = ({ width, height }: { width: number; height: number }) => (
    <svg width={width} height={height} viewBox="0 0 160 44" fill="none" xmlns="http://www.w3.org/2000/svg">
      <defs>
        ...
        <linearGradient id="silverGrad" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#e2e8f0"/><stop offset="50%" stopColor="#94a3b8"/><stop offset="100%" stopColor="#cbd5e1"/>
        </linearGradient>
  ...
  const modeSwitch = (
    <div style={{ display: 'flex', border: '1px solid #334155', borderRadius: 6, overflow: 'hidden' }}>
      <button onClick={() => handleModeSwitch('solar_citadel')} style={{ padding: '4px 8px', fontSize: 12, fontWeight: 600, cursor: 'pointer', border: 'none', backgroundColor: appMode === 'solar_citadel' ? '#854d0e' : '#1e293b', color: appMode === 'solar_citadel' ? '#fde68a' : '#64748b' }}>太陽城</button>
      <button onClick={() => handleModeSwitch('free')} style={{ padding: '4px 8px', fontSize: 12, fontWeight: 600, cursor: 'pointer', border: 'none', borderLeft: '1px solid #334155', backgroundColor: appMode === 'free' ? '#1e3a5f' : '#1e293b', color: appMode === 'free' ? '#93c5fd' : '#64748b' }}>自由</button>
    </div>
  )

  const div = <div style={{ width: 1, height: 20, backgroundColor: '#1e293b' }} />

  // スマホ用ツールバー
  if (isMobile) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0 8px', backgroundColor: '#0f172a', borderBottom: '1px solid #1e293b', userSelect: 'none', minHeight: 48 }}>
        <LogoSvg width={120} height={33} />
        <div style={{ flex: 1 }} />
        {modeSwitch}
        {div}
        <button style={canUndo() ? btnBase : { ...btnBase, opacity: 0.4 }} onClick={undo} disabled={!canUndo()}>↩</button>
        <button style={canRedo() ? btnBase : { ...btnBase, opacity: 0.4 }} onClick={redo} disabled={!canRedo()}>↪</button>
        {div}
        ...
        {div}
        <button onClick={onToggleMembers} style={{ ...btnBase, backgroundColor: membersOpen ? '#14532d' : '#1e293b', color: membersOpen ? '#86efac' : '#94a3b8', border: membersOpen ? '1px solid #22c55e' : '1px solid #334155' }}>👥</button>
      </div>
    )
  }
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0 12px', backgroundColor: '#0f172a', borderBottom: '1px solid #1e293b', flexWrap: 'wrap', userSelect: 'none', minHeight: 56 }}>
        {/* Logo */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginRight: 8 }}>
          <LogoSvg width={240} height={66} />
          <span style={{ fontSize: 10, color: '#334155', fontWeight: 500, letterSpacing: '0.05em' }}>v{__APP_VERSION__}</span>
        </div>

        {/* Mode switcher */}
        <div style={{ marginRight: 4 }}>{modeSwitch}</div>
        ...
        <button
          ...
          style={{ ...btnBase, backgroundColor: activeTool === 'area_select' ? '#1e3a5f' : '#1e293b', color: activeTool === 'area_select' ? '#93c5fd' : '#94a3b8', border: activeTool === 'area_select' ? '1px solid #3b82f6' : '1px solid #334155' }}
        >⬡ 自動配置</button>
        ...
        <button title="サイドバーを開閉" onClick={onToggleSidebar} style={{ ...btnBase, backgroundColor: sidebarOpen ? '#1e3a5f' : '#1e293b', color: sidebarOpen ? '#93c5fd' : '#94a3b8', border: sidebarOpen ? '1px solid #3b82f6' : '1px solid #334155' }}>≡</button>
        <button title="メンバーリストを開閉" onClick={onToggleMembers} style={{ ...btnBase, backgroundColor: membersOpen ? '#14532d' : '#1e293b', color: membersOpen ? '#86efac' : '#94a3b8', border: membersOpen ? '1px solid #22c55e' : '1px solid #334155' }}>👥</button>
      </div>
